...
On this page, you will use the MSET9 script, which is used to trigger MSET9. While the script is in progress, user data will temporarily disappear but will return upon the completion of this page. If you get an error when running the script, the solution to that error can most likely be found on the [troubleshooting guide](troubleshooting-mset9).
...
1. After reviewing the disclaimer, type `1` again and press Enter to accept it.
    * If you get an error, check the [troubleshooting guide](troubleshooting-mset9), then try again.
1. If you see the message `Created hacked ID1.`, press Enter to close the MSET9 script.
    * Your console will appear to lose most data / no user-installed apps on HOME Menu. **This is expected.** Your data will return at a later step.
...
1. Type the number corresponding to your console model and version, then press Enter.
    ...
    * If the window says [Not ready - check MSET9 status for more details](/images/screenshots/mset9/mset9-not-ready.png):
        ...
        * For more information, check the [troubleshooting guide](troubleshooting-mset9).
...
In this section, you will remove MSET9 to prevent further issues and to restore your user data (e.g. games and themes). This will not remove the custom firmware that you have just installed.
...
1. Type `5`, then press Enter to remove MSET9.
    ...
    * If you see `FileNotFoundError: [Errno 2] No such file or directory:` followed by a long file path containing `Nintendo 3DS`, follow the corresponding section of the [troubleshooting guide](troubleshooting-mset9#mset9-application-script).
1. Press Enter to close the MSET9 script.
