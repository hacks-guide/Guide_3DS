...
3. 將 SD 卡上的 `Nintendo 3DS` 資料夾重命名為 `Nintendo 3DS_BACKUP`，然後試著開機。 If your console successfully boots, there is some issue within your `Nintendo 3DS` folder. 試著刪除 HOME 主選單的外存 (extdata)：
...
5. 遵循[CTRTransfer](ctrtransfer)中的步驟
6. 請至 [Discord 的 Nintendo Homebrew](https://discord.gg/MWxPgEp) 伺服器中尋求協助。
...
ARM11 異常處理器已被關閉，或主機尚未安裝自製韌體。 Try enabling ARM11 exception handlers:
\+ Power off your console
\+ Hold (Select)
...
1. - 下載最新版的 [TWLFix-CFW](https://github.com/MechanicalDragon0687/TWLFix-CFW/releases/latest) _(`.3dsx` 檔案)_
2. Power off your console
3. 在 SD 卡的根目錄底下建立一個新資料夾 `3ds`
4. 將 `TWLFix-CFW.3dsx` 複製到 SD 卡中的 `/3ds/` 資料夾底下
5. Reinsert your SD card into your console
6. 啟動 Homebrew Launcher
7. 於 Homebrew 清單內啟動 TWLFix-CFW 管理器
8. 按 『A』 移除損壞的 TWL 程式
...
舊版 3DS / 2DS 在做完 CTRTransfer 或區碼變更之後可能會出現這個問題。 Follow the instructions [here](region-changing#section-vi---fixing-locale-related-issues) to fix this issue (skipping steps 3, 4, 5, and 6).
...
1. Power off your console
2. 將 SD 卡插入至電腦中
3. 移動至您 SD 卡的 `/Nintendo 3DS/<ID0>/<ID1>/extdata/00000000` 資料夾中
4. 刪除對應您 3DS 區域的資料夾：
   - **EUR Region**: `00000098`
   - **JPN Region**: `00000082`
   - **USA Region**: `0000008f`
